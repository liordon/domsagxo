...
from domsagxo.compilation.definitions import GrammarVariable
...
class TestAstTimeSpans(object):
    @pytest.fixture
    def ast(self):
        return ast_bld.build(start=GrammarVariable.TIME_SPAN.value)
    ...
    def test_cannotFormatAnHourAndAnHour(self, ast):
        with pytest.raises(domsagxo.compilation.definitions.EsperantoSyntaxError):
...
class TestAstTimePoints(object):
    @pytest.fixture
    def ast(self):
        return ast_bld.build(start=GrammarVariable.TIME_POINT.value)
...
class TestAstRandomGeneration(object):
    @pytest.fixture
    def ast(self):
        return ast_bld.build(start=GrammarVariable.ROUTINE_INVOCATION.value)
...
class TestAstApplianceManagement(object):
    @pytest.fixture
    def ast(self):
        abstract_syntax_tree = ast_bld.build(
            start=GrammarVariable.ROUTINE_INVOCATION.value)
...
class TestObjectOrientedActions(object):
    @pytest.fixture
    def parser(self):
        abstract_syntax_tree = ast_bld.build(start=GrammarVariable.STATEMENT.value)
        return abstract_syntax_tree
