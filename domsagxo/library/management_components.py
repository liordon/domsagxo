...
from domsagxo.library.predefined_functions import generateRandom, digitNames
from domsagxo.library.predefined_values import ApplianceTypes, all_categories
...
class Horaro(object):
    ...
    def runSetTime(self, amount_of_time=None):
        if amount_of_time is None:
            amount_of_time = datetime.timedelta(seconds=self.time_check_interval)
        target_time = self.scheduler.timefunc() + amount_of_time.total_seconds()
    ...
    def enterAtTrigger(self, trigger_function, action, argument=(), kwargs=None):
    ...
    def repeatAtTrigger(self, trigger_function, action, argument=(), kwargs=None):
